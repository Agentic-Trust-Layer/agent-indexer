...
function decodeBase64ToBytes(b64: string): Bytes | null {
  ...
  // Remove non-base64 characters (whitespace/newlines)
  let cleaned = "";
...
import { Account, Agent, AgentMetadata, AgentRegistrationFile, AgentStats, AgentTransfer, AgentURIUpdate, Collection, GlobalStats } from "../generated/schema";
...
function fetchAndParseMetadata(agent: Agent, uri: string | null, ts: BigInt): void {
  ...
  // Always create/update a registration file stub so `agentRegistrationFiles` reflects all agents,
  // even when the URI can’t be fetched (e.g. https://...) or decoded.
...
export function handleTransfer(e: TransferEvent): void {
  ...
  if (agent == null) {
    ...
    // NOTE: In the Jan 2026 ERC-8004 spec, tokenId == agentId and tokenURI == agentURI.
    // We prefer indexing agentURI via the Registered(agentId, agentURI, owner) event.
    // If the Registered event is missed (startBlock too late), you may need to backfill by reindexing.

    // Ensure stats exist on first mint
...
export function handleRegistered(e: RegisteredEvent): void {
  ...
  // owner set from the Registered event
  const owner = getOrCreateAccount(e.params.owner);
  agent.owner = owner.id;

  // Parse & persist registration data from agentURI (only works for IPFS formats)
  fetchAndParseMetadata(agent as Agent, e.params.agentURI, e.block.timestamp);

  // Ensure stats + global counters exist
  ...
  stats.save();
  const g = getOrCreateGlobalStats(e.block.timestamp);
  if (g.totalAgents.equals(BigInt.zero())) {
    // don't guess totalAgents here; Transfer(mint) increments it. Just keep timestamp fresh.
  }
  g.save();
...
export function handleMetadataSet(e: MetadataSetEvent): void {
  ...
  // Spec reserved key: agentWallet (abi.encodePacked(address) or empty on transfer)
  if (key == "agentWallet") {
    let agent = Agent.load(agentIdStr);
    if (agent != null) {
